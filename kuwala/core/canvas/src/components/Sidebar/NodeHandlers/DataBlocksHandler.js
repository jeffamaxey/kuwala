import React, { DragEvent } from 'react';
import DataBlocksDTO from "../../../data/dto/DataBlocksDTO";
import {v4} from "uuid";
import {useStoreActions} from "easy-peasy";

export default ({onDragStart, onClickAddDataBlock, dataSource}) => {
    const {convertDataBlocksIntoElement} = useStoreActions((actions) => actions.canvas)

    const dataBlocks = new DataBlocksDTO({
        tableName: null,
        schemaName: null,
        dataSetName: null,
        dataBlockId: v4(),
        dataBlockEntityId: null,
        isConfigured: false,
        dataSourceDTO: dataSource,
        dataSourceId: dataSource.id,
        columns: [],
        name: `${dataSource.dataCatalogItemId}`,
        dataCatalogType: dataSource.dataCatalogItemId,
    });

    const getLabelByDataCatalogId = (catalogId) => {
        switch (catalogId){
            case('postgres'):
                return 'Postgres'
            case('bigquery'):
                return 'Big Query'
            default:
                return 'Invalid Label'
        }
    }

    const nodeInfo = {
        type: 'dataBlocks',
        data: {
            label: getLabelByDataCatalogId(dataBlocks.dataCatalogType),
            dataSource: dataBlocks.dataSourceDTO,
            dataBlocks: dataBlocks,
        },
        sourcePosition: 'right',
        targetPosition: 'left',
    }

    return (
        <div
            className={`
                    p-5
                    m-0
                    shadow-xl
                    rounded-lg
                    w-24
                    h-24
                    flex flex-col justify-center items-center
                    relative
            `}
            onDragStart={(event: DragEvent) => onDragStart(event, nodeInfo)}
            onClick={() => {
                console.log('Triggered on click')
                onClickAddDataBlock(dataBlocks)
                convertDataBlocksIntoElement()
            }}
            draggable
        >
            <img
                draggable={false}
                src={dataSource.logo}
                alt={`${dataSource.name} logo`}
            />
            <span className={'mt-1 font-semibold text-xs'}>{dataSource.name}</span>
            <div
                className={`
                    absolute right-0 top-0 p-1 border rounded-full w-5 h-5 -mr-2 -mt-2
                    ${dataSource.connected ? "bg-kuwala-green" : "bg-red-400"}
                `}
            />
        </div>
    )
}